...
int YaPUBImpl::receive(int &key, int &size, char **pcData, std::string &ident) {
  ...
  if (0 != mpReqRespSocket) {
    ...
    if (-1 < rc && 0 < items[0].revents) {
      int addressSize = zmq_recv(mpReqRespSocket, address,
                                 YaComponent::MaxIdentSize, ZMQ_NOBLOCK);
      if (0 < addressSize) {
        ident = address;
        ...
        // receive key
        iBytes = zmq_recv(mpReqRespSocket, mcKeyReq, YaComponent::KeySize,
                          ZMQ_NOBLOCK);
        if (0 < iBytes) {
          ...
          sscanf(mcKeyReq, YaComponent::KeyFmt, &key);
          {
            if (0 <= key) {
              ...
              rc = zmq_getsockopt(mpReqRespSocket, ZMQ_RCVMORE, &more,
                                  &moreSize);
              if (-1 < rc && more) {
                iBytes = zmq_recv(mpReqRespSocket, mMsgBufferReq.data(),
                                  YaComponent::MaxMessageSize, ZMQ_NOBLOCK);
                if (0 < iBytes) {
                  size = iBytes;
              ...
              zmq_send(mpReqRespSocket, mcKeyReq, YaComponent::KeySize,
                       ZMQ_SNDMORE);
              zmq_send(mpReqRespSocket, YaComponent::SynAck,
                       strlen(YaComponent::SynAck), 0);
              iBytes = 0;
              rc = zmq_getsockopt(mpReqRespSocket, ZMQ_RCVMORE, &more,
                                  &moreSize);
              if (-1 < rc && more) {
              ...
              iBytes = zmq_recv(mpReqRespSocket, cNotKey, YaComponent::KeySize,
                                ZMQ_NOBLOCK);
              assert((0 < iBytes) && (iBytes == YaComponent::KeySize));
              if (0 < iBytes) {
                int notKey = -1;
                sscanf(cNotKey, YaComponent::KeyFmt, &notKey);
                qDebug() << "peer " << ident.c_str()
                         << " setNotification property " << notKey;
...
void YaPUBImpl::setConnectionPara(const char *address, int hwm) {
  ...
  if (mpReqRespSocket && !mbBound) {
    ...
    int mandatory = 1;
    zmq_setsockopt(mpReqRespSocket, ZMQ_ROUTER_MANDATORY, &mandatory,
                   sizeof(mandatory));
...
int YaPUBImpl::response(int key, const ::google::protobuf::MessageLite &msg,
...
int YaPUBImpl::send(int key, int msgSize, const char *msgData) {
  ...
  if (0 != mpReqRespSocket) {
    for (std::map<std::string, std::map<int, int>>::const_iterator it =
             mPeerMap.begin();
         it != mPeerMap.end(); ++it) {
      if (1 == mPeerMap[it->first][key]) {
        rc = zmq_send(mpReqRespSocket, it->first.c_str(), it->first.length(),
                      ZMQ_SNDMORE);
        if (-1 == rc) {
          break;
        }
...
int YaPUBImpl::send(int key, int msgSize, const char *msgData,
  ...
  if (0 != mpReqRespSocket) {
    ...
    for (std::map<std::string, std::map<int, int>>::const_iterator it = itStart;
         it != itEnd; ++it) {
