...
YaSUBImpl::YaSUBImpl(void *context)
    : mpReqRespSocket(0), mbConnected(false),
      mMsgOutBuffer(YaComponent::MessageSize),
      mMsgRespBuffer(YaComponent::MaxMessageSize), miMessageCnt(0),
      mbSync(false) {
  assert(0 != context);
  if (0 != context) {
    YaComponent::sleep(1);
...
void YaSUBImpl::close() { zmq_close(mpReqRespSocket); }

YaSUBImpl::~YaSUBImpl() { zmq_close(mpReqRespSocket); }
...
int YaSUBImpl::receive(int &key, int &size, char **pcData) {
  ...
  if (0 != mpReqRespSocket) {
    ...
    if (-1 < rc && 0 < items[0].revents) {
      ...
      assert(0 == iBytes);
      iBytes =
          zmq_recv(mpReqRespSocket, mcKey, YaComponent::KeySize, ZMQ_NOBLOCK);
      ...
      if (0 <= key) {
        ...
        if (more) {
          iBytes = zmq_recv(mpReqRespSocket, mMsgRespBuffer.data(),
        ...
        if (-1 < rc && more) {
          iBytes = zmq_recv(mpReqRespSocket, mMsgRespBuffer.data(),
                            YaComponent::MaxMessageSize, ZMQ_NOBLOCK);
          if (0 < iBytes) {
            mbSync = (0 == strncmp(YaComponent::SynAck, mMsgRespBuffer.data(),
