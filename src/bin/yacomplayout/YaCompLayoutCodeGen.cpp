                                 ...
                                 const AddressMap& adresses )
{
  for ( auto* process : processes )
  {
    ...
    while ( thread )
    {
      YaComponentCore::printDbg( std::string( "write code for thread " ) + thread->Attribute( "name" ) );
      ...
      while ( component )
      {
        ...
        fhSource << "    " << component->Attribute( "name" ) << ".moveToThread(&thread->{name});" << std::endl;
...
void YaCompLayoutCodeGen::writeComponentParts( std::ofstream& fhSource, Element* component, const AddressMap& addresses )
{
  std::filesystem::path xmlfilename = component->Attribute( "xml" );

  fhSource << "    " << xmlfilename.stem().string() << " " << component->Attribute( "name" ) << "(context);" << std::endl;
  ...
  while ( provides )
  {
    ...
    while ( ifc )
    {
      fhSource << "    " << component->Attribute( "name" ) << ".setConnectionPara" << ifc->Attribute( "id" ) << "( \"" << ifc->Attribute( "address" )
               << "\"";
