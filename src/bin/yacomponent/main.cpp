...
int main( int argc, char** argv )
{
  ...
  {
    YaComponentCore::printFatal( std::string( "given component xml file " ) + componentPath.u8string() + std::string( " not found" ) );
  }
  if ( !ifcPath.empty() && !std::filesystem::exists( ifcPath ) )
  {
    YaComponentCore::printFatal( std::string( "given ifc xml file " ) + ifcPath.u8string() + std::string( " not found" ) );
  }
