                                         ...
                                         const EntryList& usedIfc )
{
  ...
  YaComponentCore::printDbg( std::string( "YaComponentCodeGen:: write component source code file " ) + sourceFilename );
  ...
  fhHeader << "class " << compName << "Impl : public QObject" << std::endl;
  fhHeader << "{" << std::endl;
  ...
  {
    fhHeader << "  void setConnectionPara" << ifc.at( "id" ) << "( const char* address, int hwm = 0 );" << std::endl;
  }

  for ( const auto& ifc : usedIfc )
  {
    fhHeader << "  void setConnectionPara" << ifc.at( "id" ) << "( const char* address, const char* ident );" << std::endl;
  }
  ...
  fhHeader << std::endl << "protected:" << std::endl;

  fhHeader << "  enum PROXY_IDS" << std::endl;
  fhHeader << "  {" << std::endl;
  fhHeader << "    PROXY_INVALID = -1";
  for ( const auto& ifc : usedIfc )
  {
    fhHeader << ",\n"
             << "    PROXY_" << YaComponentCore::to_upper( ifc.at( "id" ) );
  }
  ...
  fhHeader << "  {" << std::endl;
  fhHeader << "    STUB_INVALID = -1";

  for ( const auto& ifc : providedIfc )
  {
    fhHeader << ",\n    STUB_" << YaComponentCore::to_upper( ifc.at( "id" ) );
  }
  ...
  for ( const auto& ifc : usedIfc )
  {
    ...
    strCtorImpl +=
      "  , m" + ifc.at( "id" ) + "( context, PROXY_" + YaComponentCore::to_upper( ifc.at( "id" ) ) + ", r" + ifc.at( "classname" ) + " )\n";
  }
  ...
  for ( const auto& ifc : providedIfc )
  {
    ...
    strCtorImpl +=
      "  , m" + ifc.at( "id" ) + "( context, STUB_" + YaComponentCore::to_upper( ifc.at( "id" ) ) + ", r" + ifc.at( "classname" ) + " )\n";
  }

  fhSource << strCtorImpl;
  fhSource << "{" << std::endl;
  ...
  fhSource << "{" << std::endl;
  fhSource << "  // clang-format off" << std::endl;
  ...
  fhSource << "  // clang-format on" << std::endl;
  ...
  for ( const auto& ifc : providedIfc )
  {
    fhSource << "void " << compName << "Impl::setConnectionPara" << ifc.at( "id" ) << "( const char* address, int hwm )" << std::endl;
    fhSource << "{" << std::endl;
  ...
  for ( const auto& ifc : usedIfc )
  {
    fhSource << "void " << compName << "Impl::setConnectionPara" << ifc.at( "id" ) << "( const char* address, const char* ident )" << std::endl;
    fhSource << "{" << std::endl;
  ...
  fhHeader << "  " << compName << "Impl& operator=( const " << compName << "Impl& );" << std::endl;
  fhHeader << std::endl;
  ...
  fhSource << "} // namespace YaComponent" << std::endl; // closes the namespace
                                       ...
                                       const ElementList& includes )
{
  ...
  fhSource << ifcName + "Stub::" << ifcName << "Stub( void* context, int id, I" << ifcName << "StubHandler& rCallbackHandler )\n";
  ...
  for ( const auto* prop : properties )
  {
    ...
    strMethod += prop->Attribute( "id" );
    fhSource << "int " << ifcName << "Stub::send( int key, const " << strMethod << "& rMessage )\n{\n";
    ...
    {
      fhSource << "  auto rc = 0;\n";
      fhSource << "  if ( !google::protobuf::util::MessageDifferencer::Equals( m" << prop->Attribute( "id" ) << ", rMessage ) )\n";
      fhSource << "  {\n";
      fhSource << "    rc = mPublisher.send( key, rMessage );\n\n";
      fhSource << "    // store properties\n";
  ...
  fhSource << "          if ( -1 < key )\n";
  fhSource << "          {\n";
  fhSource << "            qFatal( \"" << ifcName << "Stub::receive() unknown key '%d'\", key );" << std::endl;
  fhSource << "          }\n";
  fhSource << "          break;\n";
  fhSource << "      }\n";
  fhSource << "    }\n";
  ...
  for ( const auto* req : requests )
  {
    ...
    while ( para )
    {
      ...
      strPara += std::string( para->Attribute( "id" ) ) + " m" + req->Attribute( "id" ) + "_" + para->Attribute( "id" ) + ";\n";
      para = para->NextSiblingElement( "para" );
    }
    ...
    while ( resp )
    {
      ...
      while ( respPara )
      {
        ...
        strPara += std::string( respPara->Attribute( "id" ) ) + " m" + req->Attribute( "id" ) + "_" + respPara->Attribute( "id" ) + ";\n";
        respPara = respPara->NextSiblingElement( "para" );
      }
      resp = resp->NextSiblingElement( "resp" );
    }
...
const tinyxml2::XMLElement* YaComponentCodeGen::findResponse( const ElementList& responses, const std::string& id )
{
