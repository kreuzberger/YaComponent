...
class YaComponentPyCodeGen
{
public:
  YaComponentPyCodeGen();
  using ElementList = std::list<const tinyxml2::XMLElement*>;
  using EntryList = std::list<std::map<std::string, std::string>>;

  void writeComponent( const std::filesystem::path& codePath, const std::string& compName, const EntryList& providedIfc, const EntryList& usedIfc );
