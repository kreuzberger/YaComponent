...
void YaComponentParser::init( const std::filesystem::path& code, const std::filesystem::path& doc, const std::string& language, bool verbose )
{
  if ( !code.empty() )
  {
...
std::tuple<YaComponentParser::ElementList, YaComponentParser::ElementList, YaComponentParser::ElementList, YaComponentParser::ElementList>
YaComponentParser::parseIfcDefinitions( Element* ifc )
{
  ...
  while ( include )
  {
    ...
    if ( file )
    {
      YaComponentCore::printDbg( std::string( "include file: " ) + std::string( include->Attribute( "file" ) ) );
