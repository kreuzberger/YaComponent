                                           ...
                                           const EntryList& usedIfc )
{
  ...
  YaComponentCore::printDbg( std::string( "YaComponentPyCodeGen:: write component source code file " ) + sourceFilename );
  ...
  {
    fhSource << "        " << YaComponentCore::to_upper( ifc.at( "id" ) ) << " = " << keyIdx++ << std::endl;
  }
                                          ...
                                          const ElementList& includes )
{
  ...
  for ( const auto* resp : responses )
  {
    ...
    while ( para )
    {
      ...
      fhSource << "                            self._" << para->Attribute( "id" ) << ".ParseFromString(msgData)" << std::endl;
    ...
    if ( !strPara.empty() )
    {
      fhSource << "                            self._callback.onResponse" << resp->Attribute( "id" ) << "(self._id" << strPara << ")" << std::endl;
    }
    else
    {
  ...
  fhSource << "                            raise RuntimeError(f\"" << ifcName << "Proxy::receive(): unknown key '{key}'\")" << std::endl;
                                         ...
                                         const ElementList& includes )
{
  ...
  fhSource << "            if key is not None:" << std::endl;
  fhSource << "                logging.info(f\"" << ifcName << "Stub::receive: key {key}\")" << std::endl;
  fhSource << std::endl;
  fhSource << "                match key:" << std::endl;
  ...
  for ( const auto* req : requests )
  {
    ...
    {
      strPara += "    ";
      strPara += std::string( para->Attribute( "id" ) ) + " self._" + req->Attribute( "id" ) + "_" + para->Attribute( "id" ) + "\n";
      para = para->NextSiblingElement( "para" );
    }
    ...
    {
      strPara += "    ";
      strPara += std::string( resp->Attribute( "id" ) ) + " m" + req->Attribute( "id" ) + "_" + resp->Attribute( "id" ) + ";\n";
      resp = resp->NextSiblingElement( "resp" );
    }
  }
...
const tinyxml2::XMLElement* YaComponentPyCodeGen::findResponse( const ElementList& responses, const std::string& id )
{
