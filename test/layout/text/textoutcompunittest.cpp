...
TextOutComp::TextOutComp(void *context)
    : YaComponent::TextOutCompImpl(
          context, static_cast<ITextGenIfcProxyHandler &>(self())),
      ITextGenIfcProxyHandler(self()), miPropertiesCnt(0),
      miResponseStartCnt(0), miResponseStopCnt(0) {
  //  setNotification(YaComponent::TextGenIfcProxy::PROP_TEXTGEN_TEXT);
}

void TextOutComp::init() { TextOutCompImpl::init(); }
